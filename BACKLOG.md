...
- [x] **M2-05 · P2 · M** — Токены лежат в открытом виде
...
**Итог вехи.** Пять пунктов закрыты. Клиент перестал падать без сети, научился говорить, что
сессия кончилась, и хранит токены там, где платформа для них и предназначена.

Дважды пришлось отступить от плана — и оба раза потому, что план называл средство, а не причину.

Для Android здесь стояло `EncryptedSharedPreferences`. Библиотека объявлена устаревшей, а
настоящая утечка была не в ней: приватные настройки и так недоступны без рута, зато в манифесте
стоял разрешённый бэкап, и файл с токенами уезжал в облако открытым текстом. Закрыто исключением
из копии и из переноса на устройство. Для iOS вместо своей обвязки над Security взят
`KeychainSettings` из `multiplatform-settings`, которая в графе зависимостей уже была: десятки
строк работы с `CFDictionary` ради написанного и проверенного не стоят того.

Посылка вехи была «клиент должен переживать отказ». Она верна, но M2-02 оказался не про это:
приложение ЗНАЛО, что сессия кончилась, и единственное, чего оно не делало, — не говорило об этом.
Экран «сервер недоступен» с обратным отсчётом предлагал ждать того, чего ждать бессмысленно.

Побочная находка: стоило вынести путь к файлу сессии в параметр, как выяснилось, что тесты писали
токены в настоящий каталог настроек машины — мешая друг другу между прогонами.

---

## M3 — Границы модулей и одна версия

- [x] **M3-01 · P2 · L** — `:shared` — не только контракт
...
- [x] **M3-06 · P3 · S** — Три копии обвязки JVM-тестов сервера
  ...
  - **Как вышло:** обвязка приложения теперь одна, а вызовов `Mongod.instance().start` осталось
    два. Второй — в `MongoCategoryRepositoryTest`, и это обвязка другого рода: ей нужен не
    сервер, а `MongoDatabase`, и общего с проводкой приложения у неё нет ничего. Формулировка AC
    оказалась уже намерения; намерение — «трёх копий проводки приложения быть не должно» —
    выполнено.
...
**Итог вехи.** Шесть пунктов закрыты. Контракт стал контрактом, версия — одна, конфигурация либо
работает, либо убрана, график вернулся на общий слой, режим разработки больше не едет в образ,
а обвязка серверных тестов существует в одном экземпляре.

Посылка вехи была мягче, чем оказалось дело. Я писал её как расхождение с эталоном — «читателю
неудобно». На деле сервер компилировал клиентские базовые классы и тянул за ними библиотеку
неизменяемых коллекций, а `MONGO_PASSWORD` читался и молча никуда не попадал: задавший пароль
получал подключение без пароля и узнавал об этом не из отказа, а из того, что всё почему-то
работает. Это не «неудобно читать», а тихо неверное поведение.

Пункт M3-06 в исходном бэклоге отсутствовал: он завёлся во время M0-05, когда я сам создал третью
копию обвязки. Записать долг пунктом в тот же момент оказалось правильным — иначе он растворился бы
в «потом приберём».

Формулировка AC у M3-06 оказалась уже намерения, и это отмечено прямо в пункте: вызовов запуска
mongod осталось два, но второй — обвязка другого рода.
...
- **`:composeApp:wasmJsTest` на сборочной машине не идёт.** Задача поднимает `ChromeHeadless`,
  которого на боксе нет: `No binary for ChromeHeadless browser on your platform`. Компиляция
  wasm при этом проходит целиком — падает только запуск браузера. В CI набор идёт (у раннеров
  GitHub Chrome предустановлен), так что проверка не потеряна, но локально её нет, и правку,
  ломающую именно wasm-тест, здесь не поймать. Ставить браузер на машину разработчика ради
  этого — отдельное решение, а не побочный шаг чужой задачи.
