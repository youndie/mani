...
- [ ] **M1-02 · P1 · S** — Регистрация принимает пустое имя и пустой пароль
...
- [ ] **M1-03 · P1 · S** — Транзакция и категория принимаются без валидации
...
- [ ] **M1-04 · P1 · S** — `POST /demo` без предела при базе на 256 MiB
...
- [ ] **M1-05 · P3 · XS** — Пароль песочницы из `kotlin.random.Random`
...
- **`:server-native:linuxX64ReleaseTest` умеет молча не запускаться.** При инкрементальном
  локальном прогоне Gradle отмечает задачу `UP-TO-DATE` даже после того, как
  `linkReleaseTestLinuxX64` пересобрал бинарь: замечено 08.09.2026 на M1-01, где правка в
  `:server-common` доехала до отладочного набора и не доехала до релизного. Сборка при этом
  зелёная, а свежими остаются результаты прошлого прогона. В CI не проявляется — там каждый раз
  чистая копия. Лечится `--rerun` на этой задаче; надёжнее смотреть на время файлов в
  `build/test-results`, а не на `BUILD SUCCESSFUL`. Пунктом не заводится: причина в поведении
  Gradle-плагина, и объём работы отсюда не виден.
