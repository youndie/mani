...
class NavGraphStabilityTest {
    ...
    fun graphIsBuiltOnce() = runComposeUiTest {
        ...
        // Барьер между изменением состояния и проверкой — `awaitIdle`, а не `waitForIdle`.
        // В браузере всё живёт в одном потоке событий: блокирующий `waitForIdle` крутится в нём
        // же и тем самым не даёт выполниться тому, чего ждёт, — а ждать есть чего, `NavHost`
        // поднимает свои корутины. На быстрой машине очередь к этому моменту обычно пуста, и
        // разницы не видно; на нагруженном раннере CI тест не укладывался в бюджет mocha.
        tick++
        awaitIdle()
        tick++
        awaitIdle()
