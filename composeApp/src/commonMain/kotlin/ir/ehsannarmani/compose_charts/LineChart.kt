...
private data class Popup(
    val properties: PopupProperties, val position: Offset, val value: Double
)
...
fun LineChart(
            ...
            if (labelProperties.labels.isNotEmpty()) {
                labelProperties.labels.maxOf {
                    textMeasurer.measure(
                        it, style = labelProperties.textStyle
                    ).size.height
    ...
    Column(modifier = modifier) {
        if (labelHelperProperties.enabled) {
            LabelHelper(
                data = data.map { it.label to it.color }, textStyle = labelHelperProperties.textStyle
            )
        ...
        Row(modifier = Modifier.fillMaxSize()) {
            ...
            CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Ltr) {
                Canvas(
                    modifier = Modifier.weight(1f).fillMaxSize().pointerInput(data, minValue, maxValue) {
                        ...
                        detectHorizontalDragGestures(onDragEnd = {
                            ...
                            data.forEach {
                                ...
                                val positionX = (change.position.x).coerceIn(
                    ...
                    if (labelProperties.enabled) {
                        labelProperties.labels.forEachIndexed { index, label ->
                            ...
                            drawText(
                                textLayoutResult = measureResult, topLeft = Offset(
                                    (size.width - measureResult.size.width).spaceBetween(
                                        itemCount = labelProperties.labels.count(), index = index
                    ...
                    data.forEachIndexed { index, line ->
                        ...
                        pathMeasure.getSegment(
                            0f, pathMeasure.length * line.strokeProgress.value, segmentedPath
                        )
...
private fun Indicators(
    ...
    Column(
        ...
        split(
            count = indicatorProperties.count, minValue = minValue, maxValue = maxValue
        ).forEach {
            BasicText(
                text = indicatorProperties.contentBuilder(it), style = indicatorProperties.textStyle
            )
        }
    }
}
...
private fun DrawScope.drawPopup(
    ...
    val measureResult = textMeasurer.measure(
        popupProperties.contentBuilder(popup.value), style = popupProperties.textStyle.copy(
    ...
    val conflictDetected =
        ((nextPopup != null) && offset.y in nextPopup.position.y - rectSize.height..nextPopup.position.y + rectSize.height) || (offset.x + rectSize.width) > size.width
    ...
    if (offsetAnimator != null) {
        val animatedOffset = Offset(
            x = offsetAnimator.first.value, y = offsetAnimator.second.value
        )
        val rect = Rect(
            offset = animatedOffset, size = rectSize
        )
        drawPath(
            path = Path().apply {
                addRoundRect(
                    RoundRect(
                        ...
                        bottomRight = CornerRadius(
                            popupProperties.cornerRadius.toPx(), popupProperties.cornerRadius.toPx()
                        ),
            ...
            }, color = popupProperties.containerColor, alpha = 1f * progress
        )
        drawText(
            textLayoutResult = measureResult, topLeft = animatedOffset.copy(
...
fun DrawScope.drawDots(
    ...
    val dotPointProperties: DotPointProperties = line.dotPointProperties ?: return
    if (dotPointProperties.enabled) {
        val radius: Float = dotPointProperties.radius.toPx() + dotPointProperties.strokeWidth.toPx() / 2
        val strokeRadius: Float = dotPointProperties.radius.toPx()
        dotPointProperties.points.forEach { i ->
            if (line.values.size > i) {
                val dotOffset = Offset(
                    x = size.width.spaceBetween(
                        itemCount = line.values.size,
                        index = i
                    ),
                    y = (size.height - calculateOffset(
                        ...
                        total = size.height,
                        value = line.values[i].toFloat()
                    )).toFloat()
                )

                drawCircle(
                    brush = dotPointProperties.color,
                    radius = radius,
                    center = dotOffset,
                    style = Stroke(width = dotPointProperties.strokeWidth.toPx()),
                )
                drawCircle(
                    brush = dotPointProperties.outlineColor,
                    radius = strokeRadius,
                    center = dotOffset,
                )
            }
    ...
    dataPoints.forEachIndexed { valueIndex, value ->
        val dotOffset = Offset(
            x = _size.width.spaceBetween(
                itemCount = dataPoints.count(), index = valueIndex
